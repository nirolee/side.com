    ...
    if (empty($member_avatar)) {
       ...
       if (file_exists(BASE_UPLOAD_PATH.DS.ATTACH_AVATAR.DS.$member_avatar)){
   			...
   			if(C('upload_service_enabled') == 1)
   				return C('upload_service_domain') . '/' . ATTACH_AVATAR . '/' .$member_avatar;
   			else
...
function getMemberAvatarForID($id){
	if(file_exists(BASE_UPLOAD_PATH.'/'.ATTACH_AVATAR.'/avatar_'.$id.'.jpg')){
		//判断图片服务器
   		if(C('upload_service_enabled') == 1)
   			return C('upload_service_domain') . '/' . ATTACH_AVATAR . '/avatar_'.$id.'.jpg';
   		else
...
function getStoreLogo($img, $type = 'store_avatar'){
    if ($type == 'store_avatar') {
        if (empty($img)) {
        	...
        	//判断图片服务器
   			if(C('upload_service_enabled') == 1)
   				return C('upload_service_domain') . '/' . ATTACH_STORE . '/' .$img;
   			else
...
function getMbSpecialImageUrl($image_name = '') {
    ...
    if(is_file(BASE_UPLOAD_PATH . $image_path)) {
        ...
        return UPLOAD_SITE_URL.'/'.defaultGoodsImage('422');
    }
}
...
function themeImagePath($param){
	//判断图片服务器
   	if(C('upload_service_enabled') == 1)
   		return C('upload_service_domain') . '/' . ATTACH_CIRCLE . '/theme/' .$param;
   	else
 ...
 * @return string
 */
function themeImageUrl($param,$size=''){
	//判断图片服务器
   	if(C('upload_service_enabled') == 1){
		if($size)
			$type = getUploadServiceType($size);

		//$name = substr($param, strpos($param, '_') + strlen('_'),   strlen($param));
		//if($name){
		//	$size = substr ( $name, 0, strpos ( $name, '.' ) );
		//	$size_arr = @explode('x',$size);
		//}

   		return C('upload_service_domain') . '/' . ATTACH_CIRCLE . '/theme/' .$param . $type;
	}else{
...
//    $site_url = empty($site_url) ? SHOP_SITE_URL : $site_url;
    ...
    $url_string = preg_replace($reg_match_from,$reg_match_to,$url_string);
   	...
   	if(strstr($url_string,'index.php?act=mb_special&op=goods_list')){
		return C('admin_site_url').'/'.$url_string;
	}else{
...
function p($array=array(),$type=0){
	@header("Content-type: text/html; charset=utf-8");
	if($type==1 || !empty($array)){
		echo "<pre>";
		print_r($array);
		exit();
	}else{
...
function getSign($array) {
  ...
  */
function set_umeng_push($member_id,$ticker,$text,$param) {
  ...
  * @param array $param 自定义参数
  */
function set_umeng_listcast_push($member_id,$ticker,$text,$param) {
   
    $member = Model('member');
    $sns_friend = Model('sns_friend');
    $mb_user_token = Model('mb_user_token');
     
    $sns_friend_array = $sns_friend -> getListFriend('friend_tomid = '.$member_id, 'friend_frommid');
    $friend_frommid_array = array();
    foreach($sns_friend_array as $k => $v) {
        
        $friend_frommid_array[] = $v['friend_frommid'];
        
    }
    $condition['member_id'] = array('in',$friend_frommid_array);
    $member_device_tokens_list  =$member -> getMemberList($condition, 'member_id,device_tokens');
    $device_tokens_android = "";
    $device_tokens_ios = "";
    foreach($member_device_tokens_list as $k => $v) {
        $condition = array();
        ...
        if(strtolower($client_type) == 'android') {
            ...
            if(empty($device_tokens_android)) {
                ...
                if(!empty($v['device_tokens'])) {
                    
                    $device_tokens_android .= $v['device_tokens'];
        ...
        if(strtolower($client_type) == 'ios') {
            ...
            if(empty($device_tokens_ios)) {
                ...
                if(!empty($v['device_tokens'])) {
                    ...
                    $device_tokens_ios .= $v['device_tokens'];
    ...
    if(!empty($device_tokens_android)) {
        ...
        $type = "listcast";
        $umengPushAPI_Android = new UmengPushAPI('574d3c2be0f55abb7a002143','nfief43ezonetaxeaguxzziegfulzu6c');
        $rs = $umengPushAPI_Android -> sendAndroidUnicast($device_tokens_android,$ticker,$text,$type,$param);
    ...
    if(!empty($device_tokens_ios)) {
        ...
        $type = "listcast";
        $umengPushAPI_IOS = new UmengPushAPI('574c0922e0f55ac362000f78','3lkyyzyymosgbij02kil7f5hqmst6s1r');
        $rs = $umengPushAPI_IOS -> sendIOSUnicast($device_tokens_ios,$ticker,$text,$type,$param);

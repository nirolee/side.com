...
function getUploadServiceType($type){
    switch (C('upload_service_type')) {
        case 'upyun':
            $type = '!' . $type;
            break;
        case 'oss':
...
function thumb($goods = array(), $type = ''){
    ...
    if(C('upload_service_enabled') == 1){
        ...
        $thumb_host = C('upload_service_domain') . '/' . ATTACH_GOODS;
        $url = $thumb_host . '/' . $store_id . '/' . $goods['goods_image'] . $type;
    }else{
...
/*
    获取编辑器图片
*/
function getEditorThumb($file, $type=''){
    //判断是否开启了图片服务器
    if(C('upload_service_enabled') == 1){
        $type = getUploadServiceType($type);
        $url = C('upload_service_domain') . '/' . $file . $type;
    }else{
        $url = UPLOAD_SITE_URL . '/' . $file;
    }
    return $url;
}
...
function cthumb($file, $type = '', $store_id = false) {
    ...
    if(C('upload_service_enabled') == 1){
        ...
        $thumb_host = C('upload_service_domain') . '/' . ATTACH_GOODS;
        $url = $thumb_host . '/' . $store_id . '/' . $file . $type;
    }else{
        $type_array = explode(',_', ltrim(GOODS_IMAGES_EXT, '_'));
        if (!in_array($type, $type_array)) {
            $type = '240';
        ...
        $thumb_host = UPLOAD_SITE_URL . '/' . ATTACH_GOODS;
        $url =  $thumb_host . '/' . $store_id . '/' . ($type == '' ? $file : str_ireplace('.', '_' . $type . '.', $file));
    }
    return $url;
}
/**
...
function brandImage($image_name = '') {
    if ($image_name != '') {
        if(C('upload_service_enabled') == 1){
            $thumb_host = C('upload_service_domain') . '/' . ATTACH_BRAND;
            return $thumb_host . '/' . $image_name;
        }else{
...
function sliderImage($image){
    if(C('upload_service_enabled') == 1){
        $thumb_host = C('upload_service_domain') . '/' . ATTACH_SLIDE;
        return $thumb_host . '/' . $image;
    }else{
        return UPLOAD_SITE_URL.'/'.ATTACH_SLIDE.DS.$image;
    }
}
 ...
 * @return [type]      [description]
 */
function getAdvImage($image) {
    if(C('upload_service_enabled') == 1){
        $thumb_host = C('upload_service_domain') . '/' . ATTACH_ADV;
        return $thumb_host . '/' . $image;
    }else{
        return UPLOAD_SITE_URL."/".ATTACH_ADV."/".$image;
    }
}

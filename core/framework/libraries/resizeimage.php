...
class ResizeImage{
	 ...
	 * @param $filling 是否允许填充空白，默认允许
         * @param string $src_type 原上传图片的类型
         * @param string $save_path 图片上传路径
	 * @return array $rs_row 返回数组形式的查询结果
	 */
	public function newImg($img, $wid, $hei,$scale,$small_ext="_small.",$dst_img='',$filling = true, $src_type = "", $save_path = ""){
...
// 		$this->config['thumb_quality'] = (C('thumb_quality') ? C('thumb_quality') : 100);
		...
		if ($this->config['thumb_type']=='im'){
			...
			$this->thumb_gd($src_type, $save_path);
		}
	}

	private function thumb_gd($src_type = "", $save_path = ""){
		$this->initi_img();
...
//			imageCopyMerge($newimg, $snewimg,$dst_x,$dst_y,0,0,$this->width*$wh/$this->height,$wh,100);
			...
			ImageJpeg ($newimg,$this->dstimg,$this->config['thumb_quality']);
                ...
                $rs =  $upload_oss->save($this->dstimg,$src_type,$save_path);
		/**
		 * 销毁临时图
		 */

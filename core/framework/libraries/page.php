...
class Page{
	...
	public function show($style = null){
		...
		switch ($this->style) {
			...
			case '2':
				$html_page .= '<ul>';
				$html_page .= '<li><span>共 '.$this->getTotalNum().' 条</span></li>';
				if ($this->getNowPage() <= 1){
			...
			case '6':
				//$html_page .= '<span>';
				if ($this->getNowPage() <= 1){
					$html_page['leftPageUrl'] = "#";
				}else {
					$html_page['leftPageUrl'] = $this->setShopPseudoStaticPageUrl($this->getNowPage()-1);
				}
				if ($this->getNowPage() == $this->getTotalPage() || $this->getTotalPage() == 0){
					$html_page['rightPageUrl'] = "#";
				}else {
					$html_page['rightPageUrl'] = $this->setShopPseudoStaticPageUrl($this->getNowPage()+1);
				}
				//$html_page .= '</span>';
				break;
			case '7':
				if ($this->getNowPage() <= 1){
					...
					$html_page .= $this->left_inside_a_html. '<a class="demo" href="'. $this->setShopPseudoStaticPageUrl('1') .'">'.$this->pre_home.'</a>'.$this->right_inside_a_html;
					$html_page .= $this->left_inside_a_html. '<a class="demo" href="'. $this->setShopPseudoStaticPageUrl($this->getNowPage()-1) .'">'.$this->pre_page.'</a>'.$this->right_inside_a_html;
				}
                                
                $html_page .= '<ul>';
				$html_page .= $this->getNowBar_myslfe();
                $html_page .= '</ul>';
				...
				if ($this->getNowPage() == $this->getTotalPage() || $this->getTotalPage() == 0){
					...
					$html_page .= $this->left_inside_a_html.'<a class="demo" href="'. $this->setShopPseudoStaticPageUrl($this->getNowPage()+1) .'">'.$this->next_page.'</a>'.$this->right_inside_a_html;
					$html_page .= $this->left_inside_a_html. '<a class="demo" href="'. $this->setShopPseudoStaticPageUrl($this->getTotalPage()) .'">'.$this->pre_last.'</a>'.$this->right_inside_a_html;
				}
	 ...
	 * @param
	 * @return string 字符串类型的返回结果
	 * update by lizh 16:25 2016/7/1
	 */
	private function getNowBar_myslfe(){
		/**
		 * 显示效果
		 ...
		 */
			...
			if($this -> getNowPage() == 1) {
				...
				if ($this->getNowPage()+3 < $this->getTotalPage()){
					
					$end = $this->getNowPage()+3;
			...
			} else if($this -> getNowPage() == 2) {
				
				if ($this->getNowPage()+2 < $this->getTotalPage()){
					
					$end = $this->getNowPage() + 2;

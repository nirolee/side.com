		...
		if(@move_uploaded_file($this->upload_file['tmp_name'],BASE_UPLOAD_PATH.DS.$this->save_path.DS.$this->file_name)){
			...
			if ($ifresize){
                            ...
                            for ($i=0;$i<count($thumb_width);$i++){
                                ...
                                $resizeImage->newImg($save_path.DS.$this->file_name,$thumb_width[$i],$thumb_height[$i],$scale[$i],$thumb_ext[$i].'.',$save_path,$this->filling,$this->upload_file['type'],$this->save_path);
                                if ($i==0) {
                        ...
                        $rs =  $upload_oss->save(BASE_UPLOAD_PATH.DS.$this->save_path.DS.$this->file_name,$this->upload_file['type'],$this->save_path);

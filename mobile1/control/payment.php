...
class paymentControl extends mobileHomeControl{
    ...
    public function notify_wxOp() {
        ...
        $wxpay = new wxpay3();
        $key = $wxpay['key'];
       if($xml['result_code'] =='SUCCESS' && $xml['sign']=  $wxpay->getSign($xml, $config['key']) ) {
...
//	fclose($fp);
     ...
     public function notifyOp() {
        // 恢复框架编码的post值
         ...
         $_POST['notify_data'] = $_POST;
        if($_POST['notify_data']) {
            //验证成功
            $result = $this->_update_order($_POST['notify_data']['out_trade_no'], $_POST['notify_data']['trade_no']);
            if($result['state']) {
                echo 'success';die;
            }
        }
    ...
    private function _update_order($out_trade_no, $trade_no) {
        ...
        if ($order_type == 'r') {
			...
			$result = $logic_payment->getCardOrderInfo($out_trade_no);
	        $result = $logic_payment->updateCardOrder($out_trade_no, $this->payment_code, $result['data'], $trade_no);

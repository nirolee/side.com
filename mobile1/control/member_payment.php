...
class member_paymentControl extends mobileMemberControl {
    ...
    public function pay_newOp() {
        ...
        if (in_array($_GET['payment_code'],array('alipay','wxpay'))) {
	...
	/**
     * 礼品卡订单支付
	 * add by lizh 17:44 2016/8/4
     */
    public function card_pay_newOp() {
		
		$url = 'http://www.site.com/mobile1/index.php?act=member_payment&op=card_pay_new&key=6345d25a2299aae4d643fa04b4be4364'
				.'&pay_sn=c790523646991352006'
				.'&password='
				.'&rcb_pay=0'
				.'&pd_pay=0'
				.'&onempf_pay=0'
        ...
        $pay_info = $this->_get_card_order_info($pay_sn,$_GET);
    ...
    private function _pd_pay($order_list, $post) {
        ...
        if ($buyer_info['member_onempf'] == 0) {
            $post['onempf_pay'] = null;
        }
        if (floatval($order_list[0]['rcb_amount']) > 0 || floatval($order_list[0]['pd_amount']) > 0 || floatval($order_list[0]['onempf_pay']) > 0) {
            return $order_list;
        }
        ...
        try {
            ...
            //使用壹积金支付
            if (!empty($post['onempf_pay'])) {
                $order_list = $logic_buy_1->onempf_pay($order_list, $post, $buyer_info);
	...
	/**
     * 站内余额支付(充值卡、预存款支付) 礼品卡订单
     * @param array order_list 取得礼品卡订单所需支付金额等信息
	 * @param array post 商品付款信息
     */
    private function _pd_card_pay($order_list, $post) {
    ...
    private function _pd_vr_pay($order_info, $post) {
        ...
        if (floatval($order_info['rcb_amount']) > 0 || floatval($order_info['pd_amount']) > 0 || floatval($order_list[0]['onempf_pay']) > 0) {
            return $order_info;
        }
        ...
        if (!empty($result['data']['order_list'])) {
            foreach ($result['data']['order_list'] as $order_info) {
                if ($order_info['order_state'] == ORDER_STATE_NEW) {
                    ...
                    $pay_amount += $order_info['order_amount'] - $order_info['pd_amount'] - $order_info['rcb_amount']- $order_info['onempf_amount'];
                    $pay_order_id_list[] = $order_info['order_id'];
                }
            }
        }
        $order =  Model('order')->getOrderInfo(array('pay_sn'=>$pay_sn),array(),'order_id');
        $order_common = Model('order')->getOrderCommonInfo(array('order_id'=>$order['order_id']));
        if ($pay_amount == 0 && $order_common['reciver_name']==''&& $order_common['reciver_info']==''&& $order_common['reciver_province_id']=='0'&&$order_common['reciver_city_id']=='0') {
              redirect(WAP_SITE_URL.'/gift/not_sent.html');
    ...
    private function _get_vr_order_info($pay_sn,$rcb_pd_pay = array()) {
        ...
        if ($result['data']['order_state'] == ORDER_STATE_NEW) {
            $pay_amount += $result['data']['order_amount'] - $result['data']['pd_amount'] - $result['data']['rcb_amount'] - $result['data']['onempf_amount'];
        }
	 ...
	 /**
      * 获取里礼品卡订单支付信息
	  * add by lizh 18:47 2016/8/4
      */
    private function _get_card_order_info($pay_sn,$rcb_pd_pay = array()) {
        $logic_payment = Logic('payment');

        //取得订单信息
        $result = $logic_payment->getCardOrderInfo($pay_sn, $this->member_info['member_id']);
		//print_r($result);
		/* print_r(ORDER_STATE_NEW);
		exit; */
        if(!$result['state']) {
            output_error($result['msg']);
        }
        ...
        //站内余额支付
        if ($rcb_pd_pay) {
            $result['data'] = $this->_pd_card_pay($result['data'],$rcb_pd_pay);
        }
        //计算本次需要在线支付的订单总金额
        $pay_amount = 0;
		$pay_amount = $result['data']['api_pay_amount'];

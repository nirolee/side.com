...
class member_orderControl extends mobileMemberControl {
    ...
    public function order_listOp() {
        ...
        $order_list_array = $model_order->getNormalOrderList($condition, $this->page, '*', 'order_id desc', '', array('order_common', 'order_goods', 'store'));
        ...
        foreach ($order_list_array as $value) {
            ...
            $value['extend_store']['store_avatar'] = getStoreLogo($value['extend_store']['store_avatar'],'store_avatar');
            //商品图
            foreach ($value['extend_order_goods'] as $k => $goods_info) {
                ...
                if ($goods_info['goods_type'] == 5) {
                    ...
                    $goods_info = $model_goods -> getGoodsInfo(array(goods_id => $value['extend_order_goods'][$k]['goods_id']),'goods_spec');
                    ...
                    $value['extend_order_goods'][$k]['goods_spec'] = implode(" ",$goods_spec);
                    } else {
                    $value['extend_order_goods'][$k]['goods_spec'] = "";    
            }
                }
            }
            ...
            $order_group_list[$value['pay_sn']]['goods_num'] = count($value['extend_order_goods']);
...
//        $condition['order_type'] = 1;
    ...
    public function order_infoOp() {
        ...
        $order_info['extend_store']['store_avatar'] = getStoreLogo($order_info['store_avatar'],'store_avatar');
...
//		output_data($data);
    ...
    public function get_expressOp() {
        ...
        $condition['order_id'] = $order_id;
        $condition['buyer_id'] = $this->member_info['member_id'];
        $order_info = $model_order->getOrderInfo($condition, array('order_common', 'order_goods'), 'order_id,e_code,shipping_code,express_inf');
        
        foreach ($order_info['extend_order_goods'] as $key => $value) {
            $order_info['extend_order_goods'][$key]['goods_image'] = cthumb($order_info['extend_order_goods'][$key]['goods_image'],240);
        }
        if($order_info['express_inf']){
        $order_info['express_inf'] = unserialize($order_info['express_inf']);
        }
        $model_express = Model('express');
        $condition = $express_info = array();
        $condition['e_code'] = $order_info['e_code'];
        $express_info = $model_express->where($condition)->order()->find();
        $order_info['e_name'] = $express_info['e_name'];
        output_data($order_info);
    }
    
    /*
     *  niro add by 2016.8.9
     */
    ...
    public function finish_orderOp(){
        ...
        $pay_info = $model_order->getNormalOrderList($condition,'','order_id,pay_sn,store_id,store_name,buyer_id,order_amount', 'order_id desc','', array('order_goods', 'order_common', 'store'));
     
        sort($pay_info);
        $order_info['reciver_info'] =$pay_info[0]['extend_order_common']['reciver_info'];
        $order_info['reciver_info']['reciver_name'] = $pay_info[0]['extend_order_common']['reciver_name'];
        $order_info['reciver_info']['mob_phone'] = $order_info['reciver_info']['mob_phone'];
        $order_info['reciver_info']['phone'] = $order_info['reciver_info']['phone'];
        $order_info['order_list'] = $pay_info;
        foreach ($order_info['order_list'] as $key => $value) {
            $order_info['order_list'][$key]['extend_store']['store_avatar'] = getStoreLogo($value['extend_store']['store_avatar'],'store_avatar');
            foreach ($order_info['order_list'][$key]['extend_order_goods'] as $k => $v) {
               $order_info['order_list'][$key]['extend_order_goods'][$k]['goods_image'] = thumb([$v],240);
            }
        }
        output_data($order_info);
    }
    ...
    public function order_delOp() {
        ...
        $condition['order_id'] = $_POST['order_id'];
            ...
            output_data(array('state'=>'fail','message'=>'暂无订单'));
        }
        $condition['delete_state'] = 0;
        $condition['buyer_id'] = $this->member_info['member_id'];
        $update = array();
        $update['delete_state'] = 1;
        $data = $model_order->editOrder($update,$condition);
        if($data){
        output_data(array('state'=>'success','message'=>'订单删除成功'));
        }else{
         output_data(array('state'=>'fail','message'=>'订单删除失败'));
        }
    }
    
        /*
     * 猜你喜欢
     * add by niro 2016/9/22
     */
    
    public function guess_likeOp() {
        $model_goods = Model('goods');
        $condition = array();
        $condition['goods_state']   = 1;
        $guess_like = $model_goods->getGoodsListByColorDistinct($condition, 'goods_id,goods_name,goods_promotion_price,goods_price,goods_image', 'rand()', '8');
        foreach ($guess_like as $key => $value) {
            $guess_like[$key]['goods_image'] = thumb($value,360);
        }
        output_data($guess_like);
    }
}

...
class member_fundControl extends mobileMemberControl {
     ...
     * 余额记录
     */
    public function onempfOp() {
		$model_onempf = Model('onempf');
		
        $condition = array();
        $condition['type'] = 'inviter';//注册
        $condition['member_id'] = $this->member_info['member_id'];
        $list = $model_onempf->getOnempfInfo($condition,'onempf_amount');
        $inviter_total = number_format(count($list),2);
        $condition = array();
        $condition['type'] = 'rebate';
        $condition['member_id'] = $this->member_info['member_id'];
        $list_rebate = $model_onempf->getOnempfInfo($condition,'onempf_amount');
        foreach ($list_rebate as $key => $value) {
          $list_rebate[$key] = $value['onempf_amount'];  
        }
        $rebate_total = number_format(array_sum($list_rebate),2);
        $member_onempf = $this->member_info['member_onempf'];
        output_data(array('inviter_total'=>$inviter_total,'rebate_total'=>$rebate_total,'member_onempf'=>$member_onempf));
    }
    ...
    public function predepositlogOp() {
       ...
       $predepoit = $this->member_info['available_predeposit'];
        output_data(array('list' => $list,'predepoit'=>$predepoit),mobile_page($page_count));
    }

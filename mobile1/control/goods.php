 ...
 */
    ...
    public function send_newsOp() {
        $model_member = Model('member');
        //随机获取5个会员头像
        $avatar_list = $model_member->getSomeMemberAvatar('5');
        foreach ($avatar_list as $k => $v) {
            $news_list[$k]['member_avatar'] = UPLOAD_SITE_URL . '/' . ATTACH_AVATAR . '/' . $v['member_avatar'];
        }
        //随机获取5个商品ID
        $model_goods = Model('goods');
        $goods_id_list = $model_goods->getSomeGoodsId('5');
        foreach ($goods_id_list as $k=>$v){
            $news_list[$k]['goods_id'] = $v['goods_id'];
        ...
        $keys = array_rand($text,5);
        
         foreach ($keys as $k=>$v){
            $news_list[$k]['text'] = $text[$v];

...
class member_refundControl extends mobileMemberControl {
    ...
    public function refund_all_postOp(){
        ...
        if (empty($order_info) || $payment_code == 'offline' || $refund['refund_id'] > 0) {//检查数据,防止页面刷新不及时造成数据错误
            ...
            if ($state) {
                ...
                output_data(array('state'=>'success','message'=>'退换保存成功'));
            } else {
    ...
    public function refund_formOp(){
        ...
        $refund_info = $model_refund->getRefundReturnInfo(array(order_id=>$order_id,order_goods_id=>$goods_id,buyer_id=>$member_id),'*');
        if(!empty($refund_info['express_id'])){
        $refund_info['express_name'] = Model()->table('express')->getfby_id($refund_info['express_id'],'e_name');
        }
        $refund_info['goods_image'] = thumb($refund_info,360);
        if(!empty($refund_info['add_time'])){
        $refund_info['add_time'] = date('Y-m-d H:i:s', $refund_info['add_time']);
        }
        if(!empty($refund_info['seller_time'])){
        $refund_info['seller_time'] = date('Y-m-d H:i:s', $refund_info['seller_time']);
        }
        if(!empty($refund_info['admin_time'])){
        $refund_info['admin_time'] = date('Y-m-d H:i:s', $refund_info['admin_time']);
        }
        $condition = array();
        $condition['buyer_id'] = $member_id;
        $condition['order_id'] = $order_id;
        ...
        if($refund_state == 1 && $goods_id > 0) {
            ...
            $refund_info['goods_price'] = ncPriceFormat($goods_info['goods_price']);
            $refund_info['goods_spec'] = $goods_info['goods_spec'];
    ...
    public function refund_postOp(){
        ...
        if($refund_state == 1 && $goods_id > 0 && empty($refund)) {
            ...
            $reason = $model_refund->getReason($condition,'reason_info');//退款退货原因

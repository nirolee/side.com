...
class memberModel extends Model {
    ...
    public function createSession($member_info = array(),$reg = false) {
		...
		if (trim($member_info['member_sinaopenid'])){
			$_SESSION['slast_key']['uid'] = $member_info['member_sinaopenid'];
                ...
                if (trim($member_info['wx_unionid'])){
			$_SESSION['wx_unionid']		= $member_info['wx_unionid'];
		}
	...
	///////////////////////////////////////////////////
	 public function register_bymobile($register_info) {
		// 注册验证
		...
		array("input"=>$register_info["mobile"],		"require"=>"true",		"message"=>'手机号码不能为空'),
		);
		...
		$insert_id	= $this->addMember_bymobile($member_info);
		if($insert_id) {
		    //添加会员积分
			if (C('points_isuse')){
	 ...
	 * @return	array 数组格式的返回结果
	 */
	public function addMember_bymobile($param) {
		if(empty($param)) {
			return false;
		}
		try {
			...
			//增加注册时默认信息 Add by MK 2016-8-19 16:42:45 start
			$condition['member_id'] = $insert_id;
			$data['member_truename'] = '玩艺网_'.$insert_id;
			$data['member_yuju'] = '我还没想好我要做什么';
			$this->editMember($condition, $data);
			//增加注册时默认信息 Add by MK 2016-8-19 16:42:45 end
		    if (!$insert) {
		        throw new Exception();
		    }
		    $this->commit();
 ...
 /**
    ...
    public function mobile_register($register_info)
    {
        ...
        $obj_validate->validateparam = array(
            ...
            array("input" => $register_info["password"], "require" => "true", "message" => '密码不能为空'),
            array("input" => $register_info["password_confirm"], "require" => "true", "validator" => "Compare", "operator" => "==", "to" => $register_info["password"], "message" => '密码与确认密码不相同'),
            array("input" => $register_info["mobile"], 'validator' => 'mobile', "mobile" => '手机号格式不正确'),
            array("input" => $register_info["mobile"], "require" => "true", "mobile" => '手机号不能为空'),
        );
        $error = $obj_validate->validate();
        if ($error != '') {
            return array('error' => $error);
        ...
        $member_info = array();
       //验证邀请码是否正确
        if($register_info['invite_mycode']){
        $member_info['inviter_id'] = Model('member')->table('member')->getfby_invite_mycode($register_info['invite_mycode'],'member_id');
         }
        $member_info['member_name'] = $register_info['username'];
        $member_info['member_passwd'] = $register_info['password'];
        $member_info['member_mobile'] = $register_info['mobile'];
        ...
        if ($insert_id) {
            ...
            if (C('points_isuse')) {
             ...
             if (C('onempf_isuse')) {
//                Model('onempf')->saveOnempfLog('regist', array('member_id' => $insert_id, 'member_name' => $register_info['username']), false);
                //添加邀请人(推荐人)会员积分 by abc.com
                $inviter_name = Model('member')->table('member')->getfby_member_id($member_info['inviter_id'], 'member_name');
           Model('onempf')->saveOnempfLog('inviter', array('member_id' => $member_info['inviter_id'], 'member_name' => $inviter_name, 'invited' => $member_info['member_name']));  
//           Model('onempf')->saveOnempfLog('inviter', array('member_id' => $register_info['inviter_id'], 'member_name' => $inviter_name, 'invited' => $member_info['member_name']));
	...
	public function addMember($param) {
		...
		try {
		    ...
		    $member_info['member_sinaopenid']	= $param['member_sinaopenid'];
		    $member_info['member_sinainfo']	= $param['member_sinainfo'];
                    $member_info['wx_info']      = $param['wx_info'];
                    $member_info['wx_unionid']	= $param['wx_unionid'];
		    //添加邀请人(推荐人)会员积分 by abc.com
		    $member_info['inviter_id']	        = $param['inviter_id'];
		    $insert_id	= $this->table('member')->insert($member_info);
			//增加注册时默认信息 Add by MK 2016-8-19 16:42:45 start
	...
	private function getCondition($conditon_array){
		...
		if($conditon_array['member_sinaopenid'] != '') {
			$condition_sql	.= " and member_sinaopenid = '{$conditon_array['member_sinaopenid']}'";
                ...
                if($conditon_array['wx_unionid'] != '') {
			$condition_sql	.= " and wx_unionid = '{$conditon_array['wx_unionid']}'";
		}
     ...
     * 保存7天
     * @param array $member_info 会员信息
	 * add by lizh 11:07 2016/7/15
     */
    public function createCookie($member_info = array()) {
        ...
        $client_ip = md5(str_replace(".","",$client_ip));
        
        if (empty($member_info) || !is_array($member_info))
            return ;
        
        $member_info_str = serialize($member_info);
        setNcCookie($client_ip,$member_info_str,7*24*3600);
     ...
     */
    public function getSomeMemberAvatar($limit='10'){
        $model = Model();
        $data = $model->query("SELECT `member_avatar` FROM `wantease_member`WHERE `member_id` >= (SELECT floor(RAND() * (SELECT MAX(`member_id`) FROM `wantease_member`))) AND `member_avatar`!='' ORDER BY `member_id` LIMIT $limit");
        return $data;
    }

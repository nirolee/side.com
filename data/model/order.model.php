...
class orderModel extends Model {
	...
	/**
	 * 取单条订单
	 * add by lizh 2016/8/10 14:27
	 */
	public function getOneList($condition, $fields = '*') {
		
		$order_info = $this->table('order')->field($fields)->where($condition)->find();
		return $order_info;
    ...
    /**
     * 主表和扩展表
     * niro add by2016.8.11
     */
    
    public function getOrderAndOrderCommonList($condition, $field = 'order.*', $page = 0, $order = 'order_id desc') {
        return $this->table('order,order_common')->join('left')->on('order_common.order_id=order.order_id')->where($condition)->field($field)->page($page)->order($order)->select();
    }
    /**
	...
	//设置欢乐送订单的的校验码
	public function set_sign_key($post,$order_id) {
	...
	//获取欢乐送订单的的校验码
	public function get_sign_key($post) {

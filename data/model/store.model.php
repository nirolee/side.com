...
class storeModel extends Model {
     ...
     * @param array $condition 查询条件
     * @param string $field 字段
	 * @store_state : 店铺状态：1 开启；0未开启
     * @return array
	 * add by lizh 16:292 2016/7/16
     */
    public function getStoreOneList($condition,$field = '*') {
    ...
    /*
     * add by lbb 2016/09/22
     */
     public function getStoreOnlineList_1_5_4($condition, $page = null, $order = '', $field = '*',$limit = '') {
        $condition['store_state'] = 1;
    ...
    public function editStore($update, $condition) {
        //清空缓存
        $store_list = $this->getStoreList($condition,'store_id');
       if (empty($store_list)) {
            return true;
        }
         $storeid_array = array();
         ...
         return $this->editStoreById($update, $storeid_array);
    }

    public function editStoreById($update, $storeid_array) {
        if (empty($storeid_array)) {
            return true;
        }
        $condition['store_id'] = array('in', $storeid_array);   
        $result = $this->table('store')->where($condition)->update($update);
    ...
    private function getGoodsListBySales($store_array) {
        $model = Model();
        $field = 'goods_id,store_id,goods_name,goods_image,goods_price,goods_promotion_price,goods_salenum';
        foreach ($store_array as $value) {
     ...
     * @param array $store_array 店铺数组
     * @return array $store_array 包含近期销量和8个推荐商品的店铺数组
     * add by lizh 2016/9/9 10:45
     * version 1.5.3
     */
    public function getStoreSearchList_1_5_3($store_array) {
        $store_array_new = array();
        if (!empty($store_array)) {
            $model = Model();
            $no_cache_store = array();
            ...
            if (!empty($no_cache_store)) {
                ...
                //获取店铺推荐商品
                $no_cache_store = $this->getGoodsListBySales_1_5_3($no_cache_store);
                //写入缓存
                foreach ($no_cache_store as $value) {
                    wcache($value['store_id'], $value, 'store_search_info');
                    
                    $store_array_new[] = $value;
                }
     ...
     * @param array $store_array 店铺数组
     * @return array $store_array 包含近期销量和8个推荐商品的店铺数组
     * add by lbb 2016/9/23 15:30
     * version 1.5.4
     */
    public function getStoreSearchList_1_5_4($store_array) {
        $store_array_new = array();
        if (!empty($store_array)) {
            $model = Model();
            $no_cache_store = array();
            ...
            if (!empty($no_cache_store)) {
                ...
                //获取店铺推荐商品
                $no_cache_store = $this->getGoodsListBySales_1_5_4($no_cache_store);
                //写入缓存
                foreach ($no_cache_store as $value) {
                    wcache($value['store_id'], $value, 'store_search_info');
    ...
    /**
     * 获取店铺4个销量最高商品
     * add by lizh 2016/9/9 10:47
     * version 1.5.3
     */
    private function getGoodsListBySales_1_5_3($store_array) {
        $model = Model();
        $field = 'goods_id,goods_image,goods_promotion_price';
        foreach ($store_array as $value) {
            $store_array[$value['store_id']]['search_list_goods'] = $model->table('goods')->field($field)->where(array('store_id' => $value['store_id'], 'goods_state' => 1))->order('goods_salenum desc')->limit(4)->select();
        }
        return $store_array;
    }
     ...
     * version 1.5.4
     */
    private function getGoodsListBySales_1_5_4($store_array) {
        $model = Model();
        $field = 'goods_id,goods_image,goods_promotion_price';
        foreach ($store_array as $value) {
    ...
    /*
     * 根据分类ID获取9个相关热门品牌
     * add by lbb 2016/9/23 11:28
     * version 1.5.4
     */
    public function getStoreListByGoodsClassId_1_5_4($gc_id){
        
        
        $store_list = Model()->query("SELECT DISTINCT S.store_id,S.store_name,S.store_avatar FROM wantease_goods AS G LEFT JOIN wantease_store AS S ON G.store_id = S.store_id WHERE (G.gc_id_1=$gc_id OR G.gc_id_2=$gc_id OR G.gc_id_3=$gc_id) AND S.store_state=1 ORDER BY S.store_collect DESC LIMIT 9");
    ...
    /*
     * 根据店铺ID获取相关优惠券列表
     * add by lbb 2016/9/26 18:01
     * version 1.5.4
     */
    public function getStoreCouponListById($store_id){
        $model = Model();
        $field = 'p_mansong_rule.rule_id,p_mansong_rule.price,p_mansong_rule.discount';
        $on = 'p_mansong.mansong_id = p_mansong_rule.mansong_id';
        $model->table('p_mansong,p_mansong_rule')->field($field);
        $data = $model->join('left')->on($on)->where("p_mansong.store_id = $store_id and p_mansong.state = '1'")->select();

...
class brandModel extends Model {
     ...
     * @return array
     */
    public function getBrandPassedInfoByID($brand_id) {
        $condition['brand_apply'] = 1;
        $condition['brand_id'] = $brand_id;
        return $this->getBrandInfo($condition, $field, $page, $order, $limit);
    }
    ...
    public function getBrandInfoByID($brand_id) {
        ...
        $brand_info = rcache($brand_id, $prefix);
        if (empty($brand_info)) {
            $brand_info = $this->getBrandInfo(array('brand_id' => $brand_id));
            $cache = array();
            $cache['brand_info'] = serialize($brand_info);
            wcache($brand_id, $cache, $prefix, 60 * 24);
        } else {
            $brand_info = unserialize($brand_info['brand_info']);
        }
        return $brand_info;
    }
}
...
class groupbuyModel extends Model{
    ...
    public function getGroupbuyExtendInfo($groupbuy_info) {
        ...
        //查品牌信息
        $this_brand_id = Model()->table('goods')->field('*')->where(array('goods_id'=>$groupbuy_info['goods_id']))->find();
        if (!empty($this_brand_id)) {
             $this_brand_id = $this_brand_id['brand_id'];
            $this_brand_info = Model()->table('brand')->field('brand_id,brand_pic')->where(array('brand_id'=>$this_brand_id))->find();
            $groupbuy_info['brand_info'] = $this_brand_info;
        }
        //查库存信息
        $goods_storage = Model()->table('goods')->field('goods_storage')->where(array('goods_id'=>$groupbuy_info['goods_id']))->find();
        $groupbuy_info['goods_storage'] = $goods_storage['goods_storage'];
        //店铺logo
        $store_info = Model()->table('store')->field('store_avatar')->where(array('store_id'=>$groupbuy_info['store_id']))->find();
        $groupbuy_info['store_avatar'] = $store_info['store_avatar'];

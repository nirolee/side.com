...
class goodsModel extends Model{
    ...
    public function getGoodsListByRecommend($condition, $field = '*', $order = 'goods_id desc',  $page = 0) {
      $on = 'goods_common.goods_commonid = goods.goods_commonid';
       return $this->table('goods,goods_common')->field($field)->join('left')->on($on)->where($condition)->page($page)->group('goods.goods_commonid')->order($order)->select();
    }
    /**
     ...
     * 推荐商铺出售中的商品
     * 手机中的设计师品牌
     * Add by MK.Chan 2016年6月24日15:06:26
     */
    public function getIsrecommendGoods($condition,$limit){
        //SELECT c.goods_commonid,c.`store_id` ,c.`goods_name`  FROM `wantease_goods_common` AS c LEFT JOIN `wantease_goods` as g
        // ON c.`goods_commonid` =g.`goods_commonid`  WHERE ( c.store_id = '169' ) AND g.goods_state=1 ORDER BY c.`is_recommend` DESC LIMIT 5
        $condition_str = 'goods_common.store_id = ' . $condition['store_id'] . ' AND goods.goods_state = 1 ';
        $on = 'goods_common.goods_commonid = goods.goods_commonid';
        $field = 'goods_common.goods_commonid';
        $order = 'goods_common.is_recommend DESC';
        $goods_commonid_arr = $this->table('goods_common,goods')->field($field)
            ->join('left')->on($on)->where($condition_str)
            ->order($order)->limit($limit)->select();
    
        // $goods_commonid_arr = $this->table('goods_common,goods')->field("goods_common.goods_commonid")->order('is_recommend DESC')->where($condition)->limit($limit)->select();


        foreach ($goods_commonid_arr as $k=>$v){
            $goods_commonid[$k]=$v['goods_commonid'];
        }
        $goods_commonid_str = $goods_commonid?implode(',',$goods_commonid):0;
        $condition1["goods_commonid"] = array("IN",$goods_commonid_str);
        $condition1["goods_state"] = 1;


        //SELECT goods_id,goods_commonid,goods_name,goods_image FROM  wantease_goods where  goods_commonid IN(103201,103216,103217,103203,103205 ) AND goods_state =1 GROUP BY goods_commonid  ORDER BY RAND() DESC
        return $this->table('goods')->field("goods_id,store_id,goods_name,goods_image,goods_price,goods_promotion_price,goods_salenum")->group("goods_commonid")->order("RAND() DESC")->where($condition1)->limit($limit)->select();
     ...
     * @return array 二维数组
	 *
	 * add by lizh 18:02 2016/7/6
     */
    public function getStoreOpenGoodsList($condition) {
		
        $condition = $this->_getRecursiveClass($condition);
	$sql = "select * from wantease_goods wg left join wantease_store ws on wg.store_id = ws.store_id where ws.store_state = 1 and wg.goods_state = 1 and wg.gc_id_1 = " . $condition['gc_id_1'] . " order by rand() limit 0,6";
    ...
    public function getGoodsDetail($goods_id) {
        ...
        $goods_info = array_merge($result1, $result2);

        $goods_info['spec_value'] = unserialize($goods_info['spec_value']);
      //   var_dump($goods_info['spec_value']);exit;
        ...
        if (!empty($image_more)) {
            foreach ($image_more as $val) {
                ...
                $fileExists = @file_get_contents(cthumb($val['goods_image'], 500, $goods_info['store_id']), null, null, -1, 1) ? true : false;
                if($fileExists){
                    $goods_image_mobile[] = cthumb($val['goods_image'], 500, $goods_info['store_id']);
                }else{
            ...
            $fileExists = @file_get_contents(thumb($goods_info, 500), null, null, -1, 1) ? true : false;
            if($fileExists){
                $goods_image_mobile[] = thumb($goods_info, 500);
            }else{
	...
	public function getMobileBodyByCommonID($goods_commonid, $fields = 'mobile_body') {
       ...
       /*
     * 随机获取N个上架商品ID
     * add by lbb 2016/9/27 11:15
     */
    public function getSomeGoodsId($limit='10'){
        $model = Model();
        $data = $model->query("SELECT goods_id FROM wantease_goods WHERE goods_state=1 ORDER BY RAND() LIMIT $limit");
        return $data;
    }
}

...
//             QueueClient::push('flushIndexer', '');
   ...
   public function getTagCategory($keyword = '') {
	...
	/**
     * add by lizh 3:49 2016/6/29
     */
	public function getLeftCategory_new($param, $sign = 0) {
		...
		foreach ($data as $value) {
            ...
            if (!empty($gc_list[$value[0]])){   // 一级
                ...
                if (!empty($gc_list[$value[0]]['class2'][$value[1]])) { // 二级
                        ...
                        if (!$sign) {   // 取得同级分类
                            if (!empty($gc_list[$value[0]]['class2'])) {
                                foreach ($gc_list[$value[0]]['class2'] as $gc2) {
                                    $tpl_data[$value[0]]['class2'][$gc2['gc_id']]['gc_id'] = $gc2['gc_id'];
                                    $tpl_data[$value[0]]['class2'][$gc2['gc_id']]['gc_name'] = $gc2['gc_name'];
                                    if (!empty($gc2['class3']) && $gc2['gc_id'] == $value[1]) {
                                        foreach ($gc2['class3'] as $gc3) {
                                            $tpl_data[$value[0]]['class2'][$gc2['gc_id']]['class3'][$gc3['gc_id']]['gc_id'] = $gc3['gc_id'];
                                            $tpl_data[$value[0]]['class2'][$gc2['gc_id']]['class3'][$gc3['gc_id']]['gc_name'] = $gc3['gc_name'];
                                        }
                    ...
                    if (!$sign) {   // 取得同级分类
                        if (!empty($gc_list)) {
                            //foreach ($gc_list[$value[0]] as $gc1) {
								
                                $tpl_data[$gc_list[$value[0]]['gc_id']]['gc_id'] = $gc_list[$value[0]]['gc_id'];
                                $tpl_data[$gc_list[$value[0]]['gc_id']]['gc_name'] = $gc_list[$value[0]]['gc_name'];
                                if (!empty($gc_list[$value[0]]['class2'])) {
                                    foreach ($gc_list[$value[0]]['class2'] as $gc2) {
                                        $tpl_data[$gc_list[$value[0]]['gc_id']]['class2'][$gc2['gc_id']]['gc_id'] = $gc2['gc_id'];
                                        $tpl_data[$gc_list[$value[0]]['gc_id']]['class2'][$gc2['gc_id']]['gc_name'] = $gc2['gc_name'];
                                        /* if (!empty($gc2['class3'])) {
                                            foreach ($gc2['class3'] as $gc3) {
                                                $tpl_data[$gc_list[$value[0]]['gc_id']]['class2'][$gc2['gc_id']]['class3'][$gc3['gc_id']]['gc_id'] = $gc3['gc_id'];
                                                $tpl_data[$gc_list[$value[0]]['gc_id']]['class2'][$gc2['gc_id']]['class3'][$gc3['gc_id']]['gc_name'] = $gc3['gc_name'];
                                            }
                                        } */
                                    }
                                }

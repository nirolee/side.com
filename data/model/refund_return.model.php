...
class refund_returnModel extends Model {
     ...
     * @return int
     */
    public function getReason($condition, $fields = '*') {
        $reason_info = $this->table('refund_reason')->field($fields)->where($condition)->find();
        return $reason_info;
    }
    ...
    public function getReasonList($condition = array(), $page = '', $limit = '', $fields = '*') {
        $result = $this->table('refund_reason')->field($fields)->where($condition)->page($page)->limit($limit)->order('sort asc,reason_id desc')->select();
        return $result;
    }
    ...
    public function getGoodsRefundList($order_list = array(), $order_refund = 0) {
        ...
        if (!empty($order_list) && is_array($order_list)) {
            foreach ($order_list as $key => $value) {
                ...
                $order_default = $model_trade->getOrderState('order_default');// 订单状态10:未付款
                $order_paid = $model_trade->getOrderState('order_paid'); //订单状态20:已付款
...
//                } elseif ($order_state > $order_default && !empty($goods_list) && is_array($goods_list)) {//已发货后对商品操作
...
//                    }
//                }
                if ($order_state > $order_default && !empty($goods_list) && is_array($goods_list)) {//已发货后对商品操作
                    $refund = $this->getRefundState($value); //根据订单状态判断是否可以退款退货
                    foreach ($goods_list as $k => $v) {
                        $goods_id = $v['rec_id']; //订单商品表编号
                        if ($v['goods_pay_price'] > 0) {//实际支付额大于0的可以退款
     ...
     * @return array
     */
    public function getOrderList($order_condition, $order_goods_id = 0) {
        $model_order = Model('order');
        $order_info = $model_order->getOrderInfo($order_condition, array('order_common', 'store', 'order_goods'));
        $order_id = $order_info['order_id'];
    ...
    public function getRefundState($order) {
        ...
        $model_trade = Model('trade');
        $order_paid = $model_trade->getOrderState('order_paid'); //20 已付款

...
class mb_specialModel extends Model {
    ...
    private function _formatMbSpecialData($item_data, $item_type) {
        switch ($item_type) {
            case 'home1':
                ...
                $item_data['width'] = @getimagesize($item_data['image'])[0];
                $item_data['height'] = @getimagesize($item_data['image'])[1];
                break;
            case 'home2':
            case 'home4':
            ...
            default:
                ...
                foreach ((array) $item_data['item'] as $key => $value) {
                    ...
                    $value['width'] = @getimagesize($value['image'])[0];
                    $value['height'] = @getimagesize($value['image'])[1];
                    $new_item[] = $value;
                }
                $item_data['item'] = $new_item;
        }
    ...
    private function _initMbSpecialItemGoodsData($item_data, $item_type) {
        ...
        if (!empty($item_data['item'])) {
            ...
            $goods_list = $model_goods->getGoodsList($condition, 'goods_id,goods_alias,goods_name,goods_promotion_price,goods_marketprice,goods_image,goods_click');
    ...
    public function editMbSpecialItemByID($update, $item_id, $special_id) {
       $special_data  = $this->getMbSpecialByID($special_id); 
       $condition['goods_id'] = array('in',$update['item_data']['item']);
       if($special_data['promotion_type']==1){
          $goods_model = Model('goods');
          $goods_update['is_firstcut'] =1 ;
          $goods_model->editGoodsById($goods_update,$update['item_data']['item']);
       }

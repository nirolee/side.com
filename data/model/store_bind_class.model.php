...
class store_bind_classModel extends Model{
	 ...
	 *
	 */
	public function getStoreBindClassList($condition,$page= 0,$order='',$field='*'){
         ...
         * 获取店铺列表
         */
        public function getStorelist($condition,$page='',$order='',$field='*'){
           $on = 'store_bind_class.store_id = store.store_id';
           $field = 'distinct CONCAT (store_bind_class.store_id) ,'.$field; 
           $result = $this->table('store,store_bind_class')->field($field)->join('left')->on($on)->where($condition)->page($page)->order($order)->select();
            return $result;
        }
